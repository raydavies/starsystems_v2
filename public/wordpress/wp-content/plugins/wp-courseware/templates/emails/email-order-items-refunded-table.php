<?php
/**
 * Email - Order Refund Table
 *
 * This template can be overridden by copying it to yourtheme/wp-courseware/emails/email-order-items-refunded-table.php.
 *
 * @package WPCW
...
/**
 * Action: Before Order Items Refunded Table.
 *
 * @since 4.3.0
 *
 ...
 */
do_action( 'wpcw_email_before_order_items_refunded_table', $order, $admin_email, $email ); ?>
    ...
    <table class="td" cellspacing="0" cellpadding="6" style="width: 100%; font-family: 'Helvetica Neue', Helvetica, Roboto, Arial, sans-serif;" border="1">
		...
		foreach ( $order->get_order_items() as $order_item ) {
			if ( apply_filters( 'wpcw_order_item_visible', true, $order_item ) ) {
				...
				?>
                <tr class="<?php echo esc_attr( apply_filters( 'wpcw_order_item_refunded_class', 'order_item', $order_item, $order ) ); ?>">
                    <td class="td"
						...
						<?php echo apply_filters( 'wpcw_order_item_refunded_course_title', $order_item_course->get_course_title(), $order_item ); ?>
                    </td>
                    <td class="td" style="text-align:<?php echo $text_align; ?>; vertical-align:middle; border: 1px solid #eee; font-family: 'Helvetica Neue', Helvetica, Roboto, Arial, sans-serif;">
						<?php echo apply_filters( 'wpcw_order_item_refunded_price', wpcw_price( $order_item_course->get_payments_price_refunded() ), $order_item ); ?>
                    </td>
                </tr>
				<?php
			}
        ...
        <tr>
            ...
            <td class="td td-total" style="text-align:<?php echo esc_attr( $text_align ); ?>;">
				<?php echo wp_kses_post( $order->get_subtotal_refunded( true ) ); ?>
            </td>
        </tr>
		<?php if ( wpcw_taxes_enabled() ) { ?>
            <tr>
                <th class="td" scope="row" style="text-align:right;">
					<?php echo esc_html__( 'Taxes', 'wp-courseware' ); ?>
                </th>
                <td class="td" style="text-align:<?php echo esc_attr( $text_align ); ?>;">
					<?php echo wp_kses_post( $order->get_tax_refunded( true ) ); ?>
                </td>
            </tr>
		<?php } ?>
        <tr>
            ...
            <td class="td" style="text-align:<?php echo esc_attr( $text_align ); ?>;">
				<?php echo wp_kses_post( $order->get_total_refunded( true ) ); ?>
            </td>
        </tr>
        </tfoot>
    </table>
...
<?php
/**
 * Action: After Order Items Refunded Table.
 *
 * @since 4.3.0
 *
 * @param \WPCW\Models\Order $order The order object.
 ...
 * @param \WPCW\Emails\Email $emal The email object.
 */
do_action( 'wpcw_email_after_order_items_refunded_table', $order, $admin_email, $email ); ?>
