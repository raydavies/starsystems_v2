...
if ( ! class_exists( 'WPCW_Import' ) ) {
	...
	class WPCW_Import {
		...
		/**
		 * The ID of the newly created course post id.
		 * @var int
		 */
		protected $course_post_id;
		...
		public function importCourseIntoDatabase() {
			...
			// Course Post Data Keys.
			$coursePostData     = $courseData;
			$coursePostDataKeys = array( 'course_status' );
			...
			foreach ( $courseData as $courseDataKey => $courseDataItem ) {
				if ( in_array( $courseDataKey, $coursePostDataKeys ) ) {
					unset( $courseData[ $courseDataKey ] );
				}
			}
			...
			// Attempt to Create Course Post.
			$course_object = new \WPCW\Models\Course( absint( $this->course_id ) );
			if ( $course_object instanceof \WPCW\Models\Course ) {
				$post_id = $course_object->get_course_post_id();
				if ( empty( $post_id ) || 0 === $post_id ) {
					$this->course_post_id = $course_object->create_post();
				}
			}
		...
		private function addUnitsToDatabase( $unitData, $moduleID ) {
			...
			foreach ( $unitData as $singleUnit ) {
				...
				$unitmeta['unit_author']      = $unitPost['post_author'];
				...
				$unitmeta['unit_drip_type']          = isset( $singleUnit['unit_drip_type'] ) ? $singleUnit['unit_drip_type'] : '';
				$unitmeta['unit_drip_date']          = isset( $singleUnit['unit_drip_date'] ) ? $singleUnit['unit_drip_date'] : '';
				$unitmeta['unit_drip_interval']      = isset( $singleUnit['unit_drip_interval'] ) ? $singleUnit['unit_drip_interval'] : '';
				$unitmeta['unit_drip_interval_type'] = isset( $singleUnit['unit_drip_interval_type'] ) ? $singleUnit['unit_drip_interval_type'] : '';
				...
				if ( $unitID ) {
					WPCW_queue_dripfeed::updateQueueItems_unitUpdated( $unitID );
				}
		 ...
		 * @return bool|int
		 */
		public function getNewCoursePostID() {
		...
		private function loadCourseData() {
			...
			$db_courses = new DB_Courses();

			$dbdata              = array();
			$defaultColumnValues = $db_courses->get_column_defaults();
			...
			foreach ( $fieldsToProcess_course as $fieldName ) {
				...
				// Set Default Data.
				if ( empty( $dbdata[ $fieldName ] ) && isset( $defaultColumnValues[ $fieldName ] ) ) {
					$dbdata[ $fieldName ] = $defaultColumnValues[ $fieldName ];
				}
		 ...
		 */
		public function sortQuestionsByOrder( $a, $b ) {
		 ...
		 */
		public function loadQuizData_addQuestionsAssociations( $singleQuiz ) {
		...
		public static function importTrainingCourseFromXML( $xmlFileName ) {
			...
			if ( ! $xml ) {
				...
				if ( ! $errorList ) {
					...
					$newCoursePostID = $import->getNewCoursePostID();
				}
			}
			...
			return array(
				...
				'course_post_id' => $newCoursePostID,
			);
		}
	}
}
