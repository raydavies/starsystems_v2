<?php
/**
 * WP Courseware Email - New Installment Order
 *
 * @package WPCW
 * @subpackage Emails
 * @since 4.6.0
...
/**
 * Class Email_New_Installment_Order.
 *
 * @since 4.6.0
 */
class Email_New_Installment_Order extends Email {
	...
	public function __construct() {
		$this->id             = 'email_new_installment_order';
		$this->title          = esc_html__( 'New Installment Order', 'wp-courseware' );
		$this->description    = esc_html__( 'New installment order emails are sent to chosen recipient(s) when an installment payment is processed.', 'wp-courseware' );
		$this->template_html  = 'emails/admin-new-installment-order.php';
		$this->template_plain = 'emails/plain/admin-new-installment-order.php';

		parent::__construct();
	}

	/**
	 * Load Email New Installment Order.
	 *
	 * @since 4.6.0
	 */
	...
	public function get_default_subject() {
		return esc_html__( '[{site_title}] New Installment Payment Order ({order_number}) - {order_date}', 'wp-courseware' );
	}
	...
	public function get_default_heading() {
		return esc_html__( 'New Installment Payment Order', 'wp-courseware' );
	}
	...
	public function get_default_content_html() {
		...
		<p>A new installment payment order has been processed for <strong>{student_name}</strong>. Below are the details:</p>
	...
	public function get_default_content_plain() {
		$plain_text = "Hello,\r\n";
		$plain_text .= "A new installment payment order has been processed for '{student_name}. Below are the details:\r\n";
	...
	public function get_headers() {
		...
		if ( 'email_new_installment_order' === $this->get_id() && ( $this->object instanceof Order ) && $this->object->get_student_email() && ( $this->object->get_student_first_name() || $this->object->get_student_last_name() ) ) {
	...
	/**
	 *Get New Installment Order Email Merge Tags.
	 *
	 * @since 4.6.0
	 *
	...
	public function get_merge_tags() {
		...
		return apply_filters( 'wpcw_email_new_installment_order_merge_tags', $merge_tags );
	}

	/**
	 * Trigger Email New Installment Order.
	 *
	 * @since 4.6.0
	 *
